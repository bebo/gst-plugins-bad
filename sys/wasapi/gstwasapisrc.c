    ...
    while (TRUE) {
      ...
      if (hr == AUDCLNT_E_DEVICE_INVALIDATED) {
        ...
        GST_ERROR_OBJECT (self,
            "IAudioCaptureClient::AUDCLNT_E_DEVICE_INVALIDATED");
      ...
      int mask_handled =
          MAXINT ^ (AUDCLNT_BUFFERFLAGS_DATA_DISCONTINUITY |
      ...
      if (G_UNLIKELY (have_frames > want_frames)) {
        ...
        if ((self->overflow_buffer_length + save_length +
                self->overflow_buffer_ptr) > self->overflow_buffer_size) {
          GST_ERROR_OBJECT (self,
              "can't save overflow at: %i length: %i bytes want %i more bytes space is %i",
              self->overflow_buffer_ptr, self->overflow_buffer_length,
              save_length, self->overflow_buffer_size);
        } else {
          gpointer write_ptr =
              &(self->overflow_buffer[self->overflow_buffer_length]);
      ...
      hr = IAudioCaptureClient_ReleaseBuffer (self->capture_client,
          have_frames);
