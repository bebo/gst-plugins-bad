...
static GstElement *
setup_nvdec (const gchar * src_caps_str)
{
  GstElement *bin;
  GstElement *nvenc, *nvdec;
  ...
  nvdec = gst_check_setup_element ("nvdec");
  fail_unless (nvdec != NULL);

  g_object_set (nvenc, "name", "encoder", NULL);
  g_object_set (nvdec, "name", "decoder", NULL);

  gst_bin_add_many (GST_BIN (bin), nvenc, nvdec, NULL);
  fail_unless (gst_element_link_pads (nvenc, "src", nvdec, "sink"));

  targetpad = gst_element_get_static_pad (nvenc, "sink");
  ...
  targetpad = gst_element_get_static_pad (nvdec, "src");
  fail_unless (targetpad != NULL);
...
GST_START_TEST (test_decode_simple)
{
  ...
  bin =
      setup_nvdec
      ("video/x-raw,format=(string)NV12,width=(int)320,height=(int)240,framerate=(fraction)25/1");
...
static gboolean
check_nvdec_available (void)
{
  gboolean ret = TRUE;
  GstElement *nvdec;

  nvdec = gst_element_factory_make ("nvdec", NULL);
  if (!nvdec) {
    GST_WARNING ("nvdec is not available, possibly driver load failure");
    return FALSE;
  }
